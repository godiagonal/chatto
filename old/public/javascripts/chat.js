...
$(window).load(function(){
  ...
  socket.on('message', function(data){
    ...
    for(i=0;i<messages.length;i++) {
      html += '<div class="row">'
      html +=   '<div class="cell">' + moment(messages[i].date).format('HH:MM:ss') + '</div>'
      html +=   '<div class="cell">' + messages[i].userName + '</div>'
      html +=   '<div class="cell">' + messages[i].message.replace(/(\r\n|\n|\r)/gm,'<br>') + '</div>'
      html += '</div>'
    }
